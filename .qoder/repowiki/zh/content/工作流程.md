...
## 简介
本文围绕 kudig 的工作流程，系统性阐述从“数据收集”到“报告生成”的完整过程，并新增 v2.0 Go 版本的离线分析、在线诊断、规则评估等处理流程。重点包括：
- 数据收集阶段：由 reference/diagnose_k8s/diagnose_k8s.sh 以 root 权限采集系统、服务、网络、日志等诊断数据，并生成结构化的诊断目录。
- 数据分析阶段：kudig.sh 或 v2-go 的 CLI（kudig）对诊断目录进行校验，调用一系列检测函数（如 check_system_resources、check_kernel 等）扫描并分析收集到的数据文件；或通过在线模式访问 K8s API 获取实时数据。
- 异常识别阶段：检测函数通过 add_anomaly（v1）或 Issue 结构（v2）将发现的异常加入全局数组/切片，脚本对部分缺失文件采取容错策略；规则引擎支持自定义 YAML 规则评估。
- 报告生成阶段：对异常进行去重、排序，并按用户选择输出文本或 JSON 格式的最终报告；v2-go 还支持 legacy 模式兼容旧版输出。

该流程强调非侵入性：kudig.sh/CLI 仅读取数据，不修改任何系统状态。
...
- v2-go/README.md：v2.0 Go 版本说明，包含离线/在线/规则/兼容模式的使用方法与特性概览。
...
- [README.md](file://v2-go/README.md#L1-L202)
...
#### 离线分析（offline）
- 输入：由 diagnose_k8s.sh 生成的诊断目录路径。
- 数据收集：离线收集器读取 system_info、system_status、service_status、memory_info、network_info、ps_command_status、daemon_status/*、logs/* 等文件，解析节点信息与系统指标。
- 分析执行：注册表按依赖拓扑顺序执行所有支持 offline 模式的分析器，收集结果。
- 异常生成：Issue 结构体承载严重级别、中文名、英文标识、详情、位置、修复建议等。
- 报告生成：去重与排序后，按 text/json 输出；支持保存到文件。

```mermaid
flowchart TD
Start["开始离线分析"] --> Validate["校验诊断目录"]
Validate --> ReadFiles["读取关键文件与目录"]
ReadFiles --> Parse["解析节点信息与系统指标"]
Parse --> Exec["按依赖顺序执行分析器"]
Exec --> Issues["收集Issue列表"]
Issues --> DedupSort["去重与排序"]
DedupSort --> Report["生成报告(文本/JSON)"]
Report --> End["结束"]
```

图表来源
- [directory.go](file://v2-go/pkg/collector/offline/directory.go#L57-L138)
- [registry.go](file://v2-go/pkg/analyzer/registry.go#L95-L149)
- [interface.go](file://v2-go/pkg/reporter/interface.go#L10-125)
...
#### 在线诊断（online）
- 输入：kubeconfig/context/node/namespace/all-nodes 等配置。
- 数据收集：在线收集器构建 K8s 客户端，获取节点状态、事件、Pod 列表、系统组件状态、DaemonSet 等信息，以原始文本形式存入 RawFiles，供分析器使用。
- 分析执行：仅执行支持 online 模式的分析器。
- 报告生成：同离线模式。

```mermaid
sequenceDiagram
participant U as "用户"
participant CLI as "kudig online"
participant COL as "在线收集器"
participant K8S as "K8s API"
participant REG as "分析器注册表"
U->>CLI : "传入配置(--kubeconfig/--context/--node/--namespace/--all-nodes)"
CLI->>COL : "构建客户端/收集配置"
COL->>K8S : "查询节点/事件/Pod/组件/DaemonSet"
K8S-->>COL : "返回数据"
COL->>REG : "执行支持online的分析器"
REG-->>CLI : "返回Issue列表"
CLI->>CLI : "去重/排序"
CLI-->>U : "输出报告(JSON/文本)"
```
...
- [registry.go](file://v2-go/pkg/analyzer/registry.go#L107-L112)
- [interface.go](file://v2-go/pkg/reporter/interface.go#L10-125)
...
#### 规则评估（rules）
- 输入：诊断目录路径，可加载内置规则与自定义 YAML 规则文件/目录。
- 执行：规则引擎遍历规则，对文件内容或系统指标进行条件判断（file_contains、regex_match、metric_threshold、and/or 组合），命中则转换为 Issue。
- 输出：去重与排序后的报告。

```mermaid
flowchart TD
RStart["开始规则评估"] --> Load["加载内置/自定义规则"]
Load --> Collect["离线收集诊断数据"]
Collect --> Eval["规则引擎评估"]
Eval --> Issues["生成Issue列表"]
Issues --> DedupSort["去重与排序"]
DedupSort --> Report["生成报告(JSON/文本)"]
Report --> REnd["结束"]
```
...
#### 兼容模式（legacy）
- 输入：诊断目录路径。
- 执行：调用 v1.0 的 bash 分析逻辑，将结果转换为 v2-go 的 Issue 结构，生成与 v1.0 JSON 输出兼容的报告。
- 退出码：依据严重程度返回 0/1/2。
...
- v2-go 在线模式：K8s API 查询带超时设置，避免长时间等待；规则引擎与分析器执行支持上下文取消。
...
## 结论
kudig.sh 与 diagnose_k8s.sh 形成完整的“采集—分析—报告”闭环。前者以非侵入方式读取后者生成的诊断目录，通过多维检测规则识别异常，并以文本或 JSON 格式输出，辅以去重与排序，帮助运维人员快速定位问题。v2.0 Go 版本在此基础上新增在线诊断、规则评估与兼容模式，进一步扩展了适用场景与灵活性。该流程可无缝集成到标准操作程序（SOP）中，作为日常巡检与故障排查的自动化工具链。
...
- 实际使用场景与集成建议：
  ...
  - v2-go 部署：可通过 Helm Chart 与 Docker 镜像快速部署，支持离线/在线/规则/兼容多种模式。
